...
本项目是由 [Websoft9](https://www.websoft9.com) 研发的 **云原生程序** ，大大简化 CloudBeaver 复杂的安装及配置。

## 系统要求

安装本项目，确保符合如下的条件（[详细参考](https://github.com/dbeaver/cloudbeaver/wiki)）：
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r cloudbeaver
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r cloudbeaver -p
```
2.复制package包后安装

将压缩的package包复制到想要安装的服务器或虚拟机，登录 Linux，运行下面的**自动化命令**即可启动安装并显示安装结果。
```
sudo bash install-cloudbeaver
```
...
启动应用后，本地浏览器访问 URL: *`http://服务器公网IP:9093`* 进入应用。  
...
| 名称 | 端口号 | 用途 |  必要性 |
| --- | --- | --- | --- |
| cloudbeaver-server | 9093 | 浏览器访问 CloudBeaver | Y |
## 文档

[CloudBeaver 管理员手册](https://support.websoft9.com/docs/cloudbeaver)

## 企业级支持

如果需要企业级支持，请订阅我们提供的 [CloudBeaver 企业级支持版](https://apps.websoft9.com/cloudbeaver) 
